...
# Assuming the dataset has columns 'Date', 'Sub-Category', and 'Sales'
# Correctly parse the Date column
data['Order Date'] = pd.to_datetime(data['Order Date'], format='%m/%d/%Y', errors='coerce')  # Adjust format if needed
...
for subcategory in subcategories:
    ...
    forecast_summary = forecast_df.groupby(forecast_df['Date'].dt.year)['Forecasted Sales'].sum()
    forecast_results[subcategory] = forecast_summary

# Display the forecasted sales for each sub-category
print("Forecasted Sales for Each Sub Category for 2019 - 2021:")
for subcategory, summary in forecast_results.items():
    print(f"\nSub Category: {subcategory}")
    for year, sales in summary.items():
        print(f"Year {year}: {sales:.2f}")
